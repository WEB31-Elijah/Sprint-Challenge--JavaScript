...
// universities.forEach(x => unisWithUni.push(x.includes('Uni')))
...
graduates.map(x => x.university.includes('Uni') ? unisWithUni.push(x.university) : false)
...
zooAnimals.filter(x => x.population < 5 ? lowPopulationAnimals.push(x.animal_name) : false)
...
const populationTotal = zooAnimals.reduce((plus, item) => plus + item.population, 0);
